...
def view():
    ...
    # 已封禁的IP
    if ip in VIEW_BANNED:
        return '<h2>🚫 该IP已被封禁，如需解封请联系管理员</h2>', 403
    
    key = request.args.get('key', '')
    if key != VIEW_PASSWORD:
        view_attempts[ip] = view_attempts.get(ip, 0) + 1
        if view_attempts[ip] >= 8:
            VIEW_BANNED[ip] = now
            return '<h2>🚫 错误次数过多，IP已被封禁</h2>', 403
